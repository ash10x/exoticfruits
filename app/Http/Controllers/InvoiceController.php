...
class InvoiceController extends Controller
{
    ...
    public function previewInvoice($template, $color)
    {
        ...
        return view('invoice.templates.' . $template, compact('invoice', 'preview', 'color', 'img', 'settings', 'customer','font_color'));
    }
    ...
    public function invoice($invoice_id)
    {
        ...
        if($invoice)
        {
            ...
            $font_color = Utility::getFontColor($color);
            return view('invoice.templates.' . $settings['invoice_template'], compact('invoice', 'color', 'settings', 'customer', 'img','font_color'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
